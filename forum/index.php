...
if (isset($metaLocation))
    $metaLocation = str_replace(array('.', '/', '\\', 'admin_'), '', $metaLocation);
...
if (isset($_POST['action']))
    $action = $_POST['action']; elseif (isset($_GET['action']))
    $action = $_GET['action']; else
    $action = '';
if (defined('MOBILE') and !isset($custom_mobile_actions[$action]) and !in_array($action, array('', 'pmail', 'vtopic', 'vthread', 'goforum', 'userinfo', 'editmsg', 'editmsg2', 'delmsg', 'deltopic', 'tpl', 'locktopic', 'movetopic', 'movetopic2', 'advld', 'downloadfile', 'pthread', 'ptopic'))) {
...
if (isset($_POST) and sizeof($_POST) > 0 and (!isset($allowedDirectPostActions) or !in_array($action, $allowedDirectPostActions) )) {
    if (isset($_SERVER['HTTP_REFERER']))
        $httpRef = strtolower($_SERVER['HTTP_REFERER']); else
        $httpRef = '';
    if (substr_count($httpRef, strtolower($main_url)) == 0 and substr_count($httpRef, strtolower($tUrl)) == 0) {
        ...
        if (isset($allowedRefs) and is_array($allowedRefs)) {
            foreach ($allowedRefs as $a) {
                if (substr_count($httpRef, strtolower($a)) > 0) {
                    $af = TRUE;
                    break;
                }
            }
        ...
        if (!$af)
            die('Sorry, it seems like an intrusion attempt or your server doesn\'t support HTTP referrers!');
    }
}
/* --Allowing to post */
...
if (!isset($startIndex))
    $startIndex = $indexphp;
if (!isset($manualIndex))
    $manualIndex = $indexphp . 'action=manual';
if ($cookiesecure)
    $csecurejs = '1'; else
    $csecurejs = '0';
if (!isset($fIconWidth))
    $fIconWidth = 16;
if (!isset($fIconHeight))
    $fIconHeight = 16;
if (!isset($uname_maxlength))
    $uname_maxlength = 15;
...
if (!isset($user_id))
    $user_id = 0;
if (!defined('PAGE1_OFFSET'))
    define('PAGE1_OFFSET', 0);
if (isset($_GET['page']))
    $page = (integer) $_GET['page'] + 0; elseif (isset($_POST['page']))
    $page = (integer) $_POST['page'] + 0; else
    $page = PAGE1_OFFSET + 1;

if (isset($_GET['forum']))
    $forum = (integer) $_GET['forum'] + 0; elseif (isset($_POST['forum']))
    $forum = (integer) $_POST['forum'] + 0; else
    $forum = 0;
if (isset($_GET['topic']))
    $topic = (integer) $_GET['topic'] + 0; elseif (isset($_POST['topic']))
    $topic = (integer) $_POST['topic'] + 0; else
    $topic = 0;
if (isset($_POST['action']))
    $action = $_POST['action']; elseif (isset($_GET['action']))
    $action = $_GET['action']; else
    $action = '';
if (isset($_POST['csrfchk']))
    $csrfchk = $_POST['csrfchk']; elseif (isset($_GET['csrfchk']))
    $csrfchk = $_GET['csrfchk']; else
    $csrfchk = '';

if ($action != 'vthread' and $action != 'vtopic' and $page < PAGE1_OFFSET + 1)
    $page = PAGE1_OFFSET + 1;
...
if (function_exists('defineRobots'))
    ...
    function defineRobots() {
        ...
        elseif ($action == 'stats' or $action == 'userinfo' or ($action == '' and $GLOBALS['page'] > PAGE1_OFFSET + 1))
            $metaRobots = 'NOINDEX,FOLLOW';

        elseif (isset($GLOBALS['mod_rewrite']) and $GLOBALS['mod_rewrite']) {
            if ($pdcc1 and !$pdcc3) {
                if ($pdcc2 or (!$pdcc2 and $action == ''))
                    $metaRobots = 'INDEX,FOLLOW'; else
                    $metaRobots = 'NOINDEX,NOFOLLOW';
            }
            else
                $metaRobots = 'NOINDEX,NOFOLLOW';
...
if ($loginError == 0) {
    ...
    if (isset($_GET['mode']) and $_GET['mode'] == 'logout') {
        if (!defined('ARCHIVE'))
            $relocUrl = $main_url; else
            $relocUrl = $forums_url;
        deleteMyCookie();
        if (isset($metaLocation)) {
            $meta_relocate = "{$relocUrl}/{$startIndex}";
            echo ParseTpl(makeUp($metaLocation));
            exit;
        } else {
            header("Location: {$relocUrl}/{$startIndex}");
            exit;
        }
    }
    ...
    if ($user_id == 0 and $enableNewRegistrations)
        $registerButton = "<input type=\"button\" value=\"{$l_menu[2]}\" onclick=\"JavaScript:document.location='{$main_url}/{$indexphp}action=registernew'\" class=\"inputButton\" />"; else
        $registerButton = '';
    ...
    if ($actEnable == 0 or ($actEnable != 1 and !$actTrue))
        ...
        if ($action == 'vtopic' or $action == 'vthread' or $action == 'delAvatarAdmin' or ($action == '' and $viewTopicsIfOnlyOneForum == 1)) {
            if ((isset($allForumsReg) and $allForumsReg) OR ( isset($regUsrForums) and is_array($regUsrForums) and in_array($forum, $regUsrForums) and $user_id == 0)) {
                $l_anonTxt = $l_anonDisallowed;
                $anonPost = 0;
            } else {
                $l_anonTxt = $l_anonAllowed;
                $anonPost = 1;
            }
            if ($user_id == 0)
                $l_anonTxt = '<span class="txtSm"><br />' . $l_anonTxt . '</span>'; else
                $l_anonTxt = '';
        ...
        if ($user_sort == 0) {
            $sortByNew = 1;
            $sortedByT = $l_newAnswers;
            $sortByT = $l_newTopics;
        } else {
            $sortByNew = 0;
            $sortedByT = $l_newTopics;
            $sortByT = $l_newAnswers;
        }
        ...
        elseif (!isset($_POST['allForums']) and isset($_COOKIE[$cookiename . 'allForumsPwd'])) {
            $allForums = $_COOKIE[$cookiename . 'allForumsPwd'];
        } elseif (!isset($_POST['allForums']) and !isset($_COOKIE[$cookiename . 'allForumsPwd']) and isset($_SESSION['allForums']))
            $allForums = $_SESSION['allForums'];
        else
            $allForums = '';
        ...
        if ($protectWholeForum == 1) {
            if ($allForums != writeUserPwd($protectWholeForumPwd)) {
                ...
                echo ParseTpl(makeUp('protect_forums'));
                exit;
            }
        }
        ...
        if ($viewTopicsIfOnlyOneForum == 1 and ($action == '' or $action == 'vtopic' or $action == 'vthread')) {
            ...
            $forumsArray[$row[0]] = array($row[1], $row[2], $row[3], $row[4]);
            $forum = $row[0];
            if ($action == '')
                $action = 'vtopic';
        }
        ...
        if (isset($archives_url) or isset($forums_url)) {
            if (!defined('ARCHIVE'))
                $archivesLink = "<a href=\"{$archives_url}\">{$l_arcForumArchives}</a> {$l_sepr} "; else
                $archivesLink = "<a href=\"{$forums_url}/{$startIndex}\">{$l_arcBackToForums}</a> {$l_sepr} ";
        }

        /* Moderator's definition */
        $isMod = 0;
        if ($forum != 0) {
            if (isset($mods[$forum]) and in_array($user_id, $mods[$forum]))
                $isMod = 1;
        }
        else {
            foreach ($mods as $key => $val)
                if (is_array($val) and in_array($user_id, $val)) {
                    $isMod = 1;
                    break;
                }
        }
...
//topic was moved - permanent redirect here
                if (isset($mod_rewrite) and $mod_rewrite)
                    $rcurl = addTopicURLPage(genTopicURL($main_url, $topicData[4], '#GET#', $topic, $topicData[0]), $page);
                else
                    $rcurl = addGenURLPage("{$main_url}/{$indexphp}action=vthread&forum={$topicData[4]}&topic={$topic}", $page, '&');
                ...
                exit;
            } elseif (isset($_GET['goPost']) and isset($_GET['user']))
                $forum = $topicData[4];
...
//else $forb=1;
    ...
    if ($user_id != 1 and $forum != 0) {
        if (isset($clForums) and in_array($forum, $clForums)) {
            if (isset($clForumsUsers[$forum]) and !in_array($user_id, $clForumsUsers[$forum]))
                $forb = 2;
        }
        if (isset($roForums) and in_array($forum, $roForums) and $isMod != 1) {
            if (in_array($action, array('pthread', 'ptopic', 'editmsg', 'editmsg2', 'delmsg', 'locktopic', 'unlocktopic', 'deltopic', 'movetopic', 'movetopic2', 'sticky', 'unsticky')))
                $forb = 1;
        }
        if (isset($poForums) and in_array($forum, $poForums) and $isMod != 1) {
            if ($action != '' and in_array($action, array('ptopic')))
                $forb = 1;
        }
    }
...
//exit;
        ...
        if ($user_id != 1 and $banRes = db_ipCheck($thisIp, $thisIpMask, $user_id)) {
            ...
            echo ParseTpl(makeUp('main_access_denied'));
            exit;
        }
        ...
        if ($user_id != 0) {
            ...
            if (!in_array($action, array('registernew', 'register', 'sendpass', 'sendpass2')))
                $user_logging = ParseTpl(makeUp('user_login_only_form')); else
                $user_logging = '';
        }
        ...
        function getPage($topic, $totalPosts) {
            if (isset($GLOBALS['themeDesc']) and in_array($topic, $GLOBALS['themeDesc']))
                $page = PAGE1_OFFSET + 1;
            elseif ($totalPosts <= $GLOBALS['viewmaxreplys'])
                $page = PAGE1_OFFSET + 1;
            else
                $page = ceil($totalPosts / $GLOBALS['viewmaxreplys']) + PAGE1_OFFSET;
            return $page;
        }

        if ($action == 'pthread') {
            if ($reqTxt != 1)
                require_once($pathToFiles . 'bb_func_txt.php');require($pathToFiles . 'bb_func_pthread.php');
        }
        elseif ($action == 'ptopic') {
            if ($reqTxt != 1)
                require_once($pathToFiles . 'bb_func_txt.php');require($pathToFiles . 'bb_func_ptopic.php');
        }
        ...
        if ($action == 'pthread') {
            if (!isset($errorMSG)) {
                if (isset($anchor) and $anchor != '')
                    $anchor = '#msg' . $anchor; elseif ($postText == '')
                    $anchor = '#newreply'; else
                    $anchor = '';
        ...
        elseif ($action == 'vtopic') {
            if (isset($redthread) and is_array($redthread) and isset($redthread[$forum])) {
                if (isset($metaLocation)) {
                    ...
                    if (isset($mod_rewrite) and $mod_rewrite)
                        $furl = addTopicURLPage(genTopicURL($main_url, $forum, '#GET#', $redthread[$forum], '#GET#'), PAGE1_OFFSET + 1); else
                        $furl = addGenURLPage("{$main_url}/{$indexphp}action=vthread&forum=$forum&topic={$redthread[$forum]}", PAGE1_OFFSET + 1, '&');
        ...
        elseif ($action == 'ptopic') {
            ...
            if (!isset($errorMSG)) {
                if (isset($metaLocation)) {
                    ...
                    if (isset($mod_rewrite) and $mod_rewrite)
                        $furl = addTopicURLPage(genTopicURL($main_url, $forum, '#GET#', $topic, '#GET#'), PAGE1_OFFSET + 1); else
                        $furl = addGenURLPage("{$main_url}/{$indexphp}action=vthread&forum=$forum&topic=$topic", PAGE1_OFFSET + 1, '&');
                    header("Refresh: 0; url={$furl}");
                    exit;
                }
            }
        ...
        elseif ($action == 'search') {
            if ($reqTxt != 1)
                require($pathToFiles . 'bb_func_txt.php');require($pathToFiles . 'bb_func_search.php');
        }
        ...
        elseif ($action == 'locktopic')
            require($pathToFiles . 'bb_func_locktop.php');

        elseif ($action == 'editmsg') {
            $step = 0;
            require($pathToFiles . 'bb_func_editmsg.php');
        } elseif ($action == 'editmsg2') {
            require_once($pathToFiles . 'bb_func_txt.php');
            $step = 1;
            require($pathToFiles . 'bb_func_editmsg.php');
        } elseif ($action == 'delmsg') {
            $step = 0;
            require($pathToFiles . 'bb_func_delmsg.php');
        } elseif ($action == 'movetopic') {
            $step = 0;
            require($pathToFiles . 'bb_func_movetpc.php');
        } elseif ($action == 'movetopic2') {
            $step = 1;
            require($pathToFiles . 'bb_func_movetpc.php');
        } elseif ($action == 'userinfo')
            require($pathToFiles . 'bb_func_usernfo.php');

        elseif ($action == 'sendpass' and file_exists($pathToFiles . 'bb_func_sendpwd.php')) {
            $step = 0;
            require($pathToFiles . 'bb_func_sendpwd.php');
        } elseif ($action == 'sendpass2' and file_exists($pathToFiles . 'bb_func_sendpwd.php')) {
            $step = 1;
            require($pathToFiles . 'bb_func_sendpwd.php');
        } elseif ($action == 'confirmpasswd') {
            require($pathToFiles . 'bb_func_confpwd.php');
        } elseif ($action == 'stats' and file_exists($pathToFiles . 'bb_func_stats.php'))
            require($pathToFiles . 'bb_func_stats.php');

        elseif ($action == 'manual')
            require($pathToFiles . 'bb_func_man.php');

        elseif ($action == 'registernew' and ($user_id == 1 or $enableNewRegistrations)) {
            $step = 0;
            require($pathToFiles . 'bb_func_regusr.php');
        } elseif ($action == 'register' and ($user_id == 1 or $enableNewRegistrations)) {
            $step = 1;
            require($pathToFiles . 'bb_func_regusr.php');
        } elseif ($action == 'prefs' and $enableProfileUpdate) {
            $step = 0;
            require($pathToFiles . 'bb_func_editprf.php');
        } elseif ($action == 'editprefs' and $enableProfileUpdate) {
            $step = 1;
            require($pathToFiles . 'bb_func_editprf.php');
        } elseif ($action == 'unsubscribe')
            require($pathToFiles . 'bb_func_unsub.php');

        elseif ($action == 'sticky') {
            $status = 9;
            require($pathToFiles . 'bb_func_sticky.php');
        } elseif ($action == 'unsticky') {
            $status = 0;
            require($pathToFiles . 'bb_func_sticky.php');
        } elseif ($action == 'viewipuser') {
            require($pathToFiles . 'bb_func_viewip.php');
        } elseif ($action == 'banip') {
            require($pathToFiles . 'bb_func_banip.php');
        } elseif ($action == 'tpl') {
            if (isset($_GET['tplName']))
                $tplName = str_replace(array('.', '/', '\\', 'admin_'), '', $_GET['tplName']); else
                $tplName = '';
            if ($tplName != '' and file_exists($pathToFiles . 'templates/' . $tplName . '.html')) {
                ...
                if (isset($metaLocation)) {
                    ...
                    header("Location: {$main_url}/{$indexphp}");
            }
        }
...
//$listTopics=$list_topics;
                if ($list_forums != '') {
                    if ($page == PAGE1_OFFSET + 1) {
                        if (isset($startPageModern) and $startPageModern)
                            $tplPr = ParseTpl(makeUp('main_modern_layout')); else
                            $tplPr = ParseTpl(makeUp('main_last_discussions'));
                        if (!isset($pageNavExtra) or $pageNavExtra == '')
                            $tplPr = preg_replace("#<!--pageNav-->(.*?)<!--/pageNav-->#is", '', $tplPr);
                    }
                    else
                        $tplPr = ParseTpl(makeUp('main_last_discuss_more'));
                    echo $tplPr;
                }
            }
        }
