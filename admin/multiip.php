...
echo "<title>Admin: Multi-hunt IP</title>";
...
<form id="loginForm" name="ip" method="get" action="ban.php">
      <tr><td colspan="2">Ban IP address. </td></tr>
      <tr><td><input name="name" type="text" class="textfield" id="name" /></td>
      <td><input type="submit" value="Submit" /></td></tr>
</form>
...
<?php
  echo "<table>";
    ...
    $ip = $_GET['ip'];
    $multi_sql = mysql_query("SELECT * FROM connectionlog WHERE ipaddy = '$ip'");



    while($multi_row = mysql_fetch_array($multi_sql))
    {
	$tbl_name="connectionlog";
	$adjacents = 3;


	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE ipaddy = '$multi_row[2]'";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];


	$targetpage = "multiip.php?ip=$ip"; 	//your file name  (the name of this file)
	...
	/* Get data. */
	$sql = "SELECT * FROM $tbl_name WHERE ipaddy = '$multi_row[2]' ORDER BY member_id DESC LIMIT $start, $limit";
