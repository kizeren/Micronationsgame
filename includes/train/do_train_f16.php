...
while ($row = mysql_fetch_array($result)) {
    ...
    $f16_t = $f16 * 5000;
    $gold = $f16;
    $message = "You are training " . $f16 . " f16's for " . number_format($f16_t) . " " . $row[7] . ", " . number_format($culture) . " culture and " . number_format($goods) . " goods.";

    //$result2 = mysql_query("UPDATE military SET f16 = f16 + '$f16' WHERE member_id = '$id'", $db);
    ...
    mysql_query("INSERT INTO trainf16 ( member_id, f16, date, lvl, timestamp ) VALUES ('$id', '$f16', '$date', '$lvl', '$timestamp')", $db);
    mysql_query("UPDATE members SET money = money - '$f16_t' WHERE member_id = '$id'", $db);
