...
include("$directory/includes/misc/include.php");
        ...
        $result = mysql_query("SELECT * FROM resource_cost WHERE id = 2", $db);
        ...
        while($costrow = mysql_fetch_array($result))
        {
            while($resrow = mysql_fetch_array($result2))
            {
                ...
                $costfood = $costrow[2] * ($resrow[8] + 1) * 2;
                $costcult = $costrow[3] * ($resrow[8] + 1) * 2;
                $costgoods = $costrow[4] * ($resrow[8] + 1) + 0.3;
		...
		exit();
                ...
                $start_research = mysql_query("UPDATE resources SET iscultureresearch = 1 WHERE member_id = '$id'", $db);
                ...
                $result3 = mysql_query("INSERT INTO nationlog (nationid, message, date) VALUES ( '$id', 'You started research on culture level $resrow[8].', Now())", $logdb);
                $errmsg_arr[] = "You started research on culture level $resrow[8].";
	        $errflag = true;
