...
include("$directory/includes/misc/include.php");
        ...
        $result = mysql_query("SELECT * FROM resource_cost WHERE id = 4", $db);
        ...
        while($costrow = mysql_fetch_array($result))
        {
            while($resrow = mysql_fetch_array($result2))
            {
                ...
                $costfood = $costrow[2] * $resrow[14];
                $costcult = $costrow[3] * $resrow[14];
                $costgoods = $costrow[4] * $resrow[14];
		...
		exit();
                ...
                $start_research = mysql_query("UPDATE resources SET isstorageresearch = 1 WHERE member_id = '$id'", $db);
                ...
                $result3 = mysql_query("INSERT INTO nationlog (nationid, message, date) VALUES ( '$id', 'You started research on storage level $resrow[14].', Now())", $logdb);
                $errmsg_arr[] = "You started research on storage level $resrow[14].";
	        $errflag = true;
